...
class GameDatasetFromDataPoints(Dataset):
    ...
    def __init__(self, raw_data, embedding_by_property, debug=False):
        ...
        self.NULL = self.num_attr_vals * self.num_attributes # idx for <NULL>
        self.SEP = self.num_attr_vals * self.num_attributes + 1 # idx for <SEP>
        self.SOS = self.num_attr_vals * self.num_attributes + 2 # idx for <SOS>
        self.EOS = self.num_attr_vals * self.num_attributes + 3 # idx for <EOS>
        self.PAD = self.num_attr_vals * self.num_attributes + 4 # idx for <PAD>
    ...
    def compute_gt(self, query_idx):
        ...
        q1_idx, q2_idx = self.decode_query_idx(self.num_attributes, self.num_attr_vals, query_idx)
        query1 = self.decode_key_idx(self.num_attributes, self.num_attr_vals, q1_idx)
        query2 = self.decode_key_idx(self.num_attributes, self.num_attr_vals, q2_idx)
        
        gt = [
            float(check_q1q2k_match(self.num_attributes, self.num_attr_vals, query1, query2, k_idx) > 0) \
...
class GameDatasetTrainDataset(GameDatasetFromDataPoints):
    ...
    def __getitem__(self, idx):
        ...
        if self.embedding_by_property:
            yj1, yj2 = self.decode_query_idx(self.num_attributes, self.num_attr_vals, y_j)
            yj1_vocab_tokens = self.decode_key_to_vocab_token(self.num_attributes, self.num_attr_vals, yj1, self.NULL)
            yj2_vocab_tokens = self.decode_key_to_vocab_token(self.num_attributes, self.num_attr_vals, yj2, self.NULL)
            x_i_vocab_tokens = self.decode_key_to_vocab_token(self.num_attributes, self.num_attr_vals, x_i, self.NULL)
        
        if self.debug:
            yj1, yj2 = self.decode_query_idx(self.num_attributes, self.num_attr_vals, y_j)
            print(
                'query\n', y_j, "\n", yj1, yj2, "\n",
                self.decode_key_idx(self.num_attributes, self.num_attr_vals, yj1), 
                self.decode_key_idx(self.num_attributes, self.num_attr_vals, yj2), "\n",
                self.decode_key_to_vocab_token(self.num_attributes, self.num_attr_vals, yj1, self.NULL),
                self.decode_key_to_vocab_token(self.num_attributes, self.num_attr_vals, yj2, self.NULL)
            )
            print('key\n', x_i, self.decode_key_idx(self.num_attributes, self.num_attr_vals, x_i), self.decode_key_to_vocab_token(self.num_attributes, self.num_attr_vals, x_i, self.NULL))
            print('all matches \n', [self.decode_key_idx(self.num_attributes, self.num_attr_vals, i) for i,v in enumerate(gt) if v == 1.])
...
class GameTestFullDataset(GameDatasetFromDataPoints):
    ...
    def __getitem__(self, idx):
        ...
        if self.embedding_by_property:
            yj1, yj2 = self.decode_query_idx(self.num_attributes, self.num_attr_vals, y_j)
            yj1_vocab_tokens = self.decode_key_to_vocab_token(self.num_attributes, self.num_attr_vals, yj1, self.NULL)
            yj2_properties = self.decode_key_to_vocab_token(self.num_attributes, self.num_attr_vals, yj2, self.NULL)
...
class PropertyBatchFetcher(GameDatasetFromDataPoints):
    ...
    def make_batch_from_propertylist(self, queries, keys):
        ...
        if queries:
            ...
            if self.embedding_by_property:
                # shape (b, num attr)
                y_j1_vocab_tokens = np.array([decode_key_properties_to_vocab_token(self.num_attributes, self.num_attr_vals, q[0], self.NULL) for q in queries])
                # shape (b, num attr)
                y_j2_vocab_tokens = np.array([decode_key_properties_to_vocab_token(self.num_attributes, self.num_attr_vals, q[1], self.NULL) for q in queries])
        ...
        if keys:
            ...
            if self.embedding_by_property:
                # shape (b, num attr)
                x_i_vocab_tokens = np.array([decode_key_properties_to_vocab_token(self.num_attributes, self.num_attr_vals, np.array(k), self.NULL) for k in keys])
                # shape (b, num attr + 1)
