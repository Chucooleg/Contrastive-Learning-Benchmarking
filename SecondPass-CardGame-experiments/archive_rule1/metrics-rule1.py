...
class ThresholdedMetrics(nn.Module):
    ...
    def breakdown_errors_by_rank(self, X_query, corrects, scores, threshold): # TODO remove scores and threshold
        ...
        num_key_hits = [
            check_if_query_key_match_by_idx(
                self.num_attributes, self.num_attr_vals, X_query_list[batch_i].item(), key_idx
        ...
        num_shared_attributes = [
            query_idx_has_real_matches(
                self.num_attributes, self.num_attr_vals, X_query_list[batch_i].item()
        ...
        make_counts = lambda : {shared_attr:{key_hit:0 for key_hit in range(self.num_attributes+1)} for shared_attr in range(self.num_attributes+1)}
        ...
        for s in range(self.num_attributes+1):
            for k in range(self.num_attributes+1):
    ...
    def forward(self, X_query, scores, threshold, X_keys, breakdown_null_nonNull=False, breakdown_byrank=False, debug=False):
        ...
        if breakdown_null_nonNull:
            ...
            # NOTE: Not great
            fil = torch.tensor(
                [query_idx_has_real_matches(self.num_attributes, self.num_attr_vals, qId.item()) > 0 \
