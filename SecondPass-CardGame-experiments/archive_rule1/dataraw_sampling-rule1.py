...
def decode_query_idx_to_key_properties(num_attributes, num_attr_vals, query_idx):
    key1_idx, key2_idx = decode_query_idx(num_attributes, num_attr_vals, query_idx)
    return [decode_key_idx(num_attributes, num_attr_vals, key1_idx), decode_key_idx(num_attributes, num_attr_vals, key2_idx)]

def decode_query_idx(num_attributes, num_attr_vals, query_idx):
...
def encode_query_idx(num_attributes, num_attr_vals, card1_idx, card2_idx):
...
def queryidx_to_querypair(num_attributes, num_attr_vals, query_idx):  
    q1_idx, q2_idx = decode_query_idx(num_attributes, num_attr_vals, query_idx)
    query_part1 = decode_key_idx(num_attributes, num_attr_vals, q1_idx)
    query_part2 = decode_key_idx(num_attributes, num_attr_vals, q2_idx)
    return query_part1, query_part2
    
def check_if_query_key_match_by_idx(num_attributes, num_attr_vals, query_idx, key_idx):
    '''return an int'''
    query_part1, query_part2 = queryidx_to_querypair(num_attributes, num_attr_vals, query_idx)
    return check_q1q2k_match(num_attributes, num_attr_vals, query_part1, query_part2, key_idx)

def check_q1q2k_match(num_attributes, num_attr_vals, query_part1, query_part2, key_idx):
    if key_idx == num_attr_vals ** num_attributes: # null key card
...
def query_idx_has_real_matches(num_attributes, num_attr_vals, query_idx):
    query_part1, query_part2 = queryidx_to_querypair(num_attributes, num_attr_vals, query_idx)
    return query_has_real_matches(query_part1, query_part2)

##################################################################

def sample_query_key_idx(num_attributes, num_attr_vals):
...
def sample_valid_query_key_idx(num_attributes, num_attr_vals):
    valid = False
    trials = 0
    while not valid:
        tmp_query_idx, tmp_key_idx = sample_query_key_idx(num_attributes, num_attr_vals)
        valid = check_if_query_key_match_by_idx(num_attributes, num_attr_vals, tmp_query_idx, tmp_key_idx)
        if valid: valids = (tmp_query_idx, tmp_key_idx)
        trials += 1
    return trials, valids

def sample_N_datapoints(num_attributes, num_attr_vals, N):
    all_valids = []
    cts = 0
    for i in tqdm(range(N)):
        trials, valids = sample_valid_query_key_idx(num_attributes, num_attr_vals)
        cts += trials
...
def sample_dataset(num_attributes, num_attr_vals, N_train, N_val):
    start_time = time.time()

    N = N_train + N_val
    datapoints, sparsity_estimate = sample_N_datapoints(num_attributes, num_attr_vals, N)
        
    data = {
        'num_attributes':num_attributes,
        'num_attr_vals':num_attr_vals,
        'key_support_size': num_attr_vals**num_attributes + 1,
        'query_support_size': (num_attr_vals**num_attributes)**2,
